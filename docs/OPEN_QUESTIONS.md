...
Частично реализовано: `WindowsForegroundFocusTracker` (`Clipensk.Windows/Input/`) захватывает
...
экземпляра `JournalWindow`, никогда на диске. Восстановление подключено в `OnAppWindowClosing` —
единственном существующем сегодня триггере скрытия окна (закрытие крестиком сворачивает в трей, не
закрывает процесс): перед `sender.Hide()` окно прячется, затем выполняется попытка
`SetForegroundWindow`, а `TryRestoreForeground` тихо ничего не делает, если `IsWindow` не подтвердит
существование окна или сам вызов не сработает (в том числе для окна с более высоким integrity
level — ограничение UIPI, обходить не нужно).
...
Не реализовано (в разработке): в коде сегодня нет ни обработки клавиши Escape, ни auto-hide по
потере активации — единственный способ скрыть окно остаётся кнопка-крестик (`OnAppWindowClosing`).
Restoration уже сработает для любого нового триггера, который тоже будет скрывать окно через
`sender.Hide()` в этом же обработчике. При реализации учитывается риск ложного срабатывания:
auto-hide по потере активации не должен прятать окно, пока открыт системный
`FileOpenPicker`/`FolderPicker` (используется в загрузке файла перевода и выборе каталога данных) —
иначе открытие пикера будет ошибочно скрывать журнал.
- режим «вставить как plain text» независимо от сохранённых форматов.
