...
Раз от MSIX отказались (§3, решение 2026-09-21), manifest-based enforcement минимальной версии ОС не
появится сам по себе — runtime-проверку нужно писать явным кодом.

**Решение пользователя (2026-09-23): при запуске на Windows 10 (сборка ниже 22000) Clipensk
показывает предупреждение, что версия Windows не поддерживается и работа не гарантируется, и
продолжает работу.**

Реализовано: `SupportedWindowsVersion` (Core) — сборка 22000 и выше; `App.OnLaunched` сразу после
захвата single-instance показывает предупреждение (`WindowsStartupMessage.ShowWarning`, ключ
`Startup.UnsupportedWindows`) и продолжает старт. Ручная проверка на Windows 10 — `UNVERIFIED`.
...
**Решение пользователя (2026-09-23): вариант 1 — ручная копия с блокировкой** («Настройки» →
«Создать резервную копию…») вместе с действием «Открыть хранилище из папки…» для восстановления.
Горячая копия (вариант 2) и автоматические копии (вариант 3) не выбраны. Протокол —
`BACKUP_PROTOCOL.md`.

Прежнее требование — файлы доступны стороннему процессу для чтения и копирования — сохраняется:
посторонняя программа может копировать каталог данных, пока Clipensk заблокирован или закрыт.

**Анализ вариантов (2026-09-23, на основе которого принято решение).** Исходные факты:
...
**Решение пользователя (2026-09-23): оставить как есть, вопрос закрыт.** Отдельных правил
совместимости версий и подписи/проверки сторонних переводов нет: недостающие в файле перевода строки
берутся из встроенного русского, лишние ключи игнорируются.
...
прийти в двух написаниях. На NTFS это один и тот же файл.

Что сделано без решения (разрешено §5 как подсказка пользователю): во всех списках приложения с
одинаковым именем показываются с путём в скобках, так что видно, откуда взялся второй. Причина
дубля в прогоне 2026-09-24 пока `UNVERIFIED`: две разные установки или одно приложение в двух
написаниях — покажет пункт N7 повторной проверки.

Варианты:

- **A. Сравнивать путь без учёта регистра** (как Windows сравнивает пути файлов). Новые написания
  одного пути находят уже известное приложение; уже созданные дубли остаются, их объединяют
  переносом в одну группу (§11). Риск: на каталогах с включённой чувствительностью к регистру (редкий
  режим NTFS для WSL) два разных файла, отличающиеся только регистром, станут одним приложением.
- **B. Оставить как есть**: дубли по регистру объединяются только переносом в одну группу.
