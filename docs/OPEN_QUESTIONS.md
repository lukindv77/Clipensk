...
- процедуру смены пароля (решено: перешифровка всех баз; не реализовано);
- действие «Начать текущую базу заново» при потере `current.db` (решено; не реализовано);
- удаление Catalog quarantine backups по сроку корзины (решено; не реализовано).
...
3. **Потеря `current.db`.** Current хранит policy, группы, identity приложений и свежую историю;
   ...
   Пароль при этом проверяется по уцелевшим базам (`CRYPTOGRAPHY.md` §4). Не реализовано.
4. **Карантин Catalog.** `Current/CatalogQuarantine` копит прежние Catalog после каждой замены.
   **Решение:** копии удаляются по тому же сроку, что и корзина внешних файлов
   (`TrashRetentionDays`, по умолчанию 30 дней). Не реализовано.
