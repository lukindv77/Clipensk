...
**Анализ вариантов и решения пользователя (2026-09-23).** Решены пункты 3 и 4; пункты 1 и 2
пользователь решил сначала подробно обсудить (зачем вообще хранится производная пароля), до этого
они остаются открытыми.

1. **Смена пароля.** Сейчас MasterKey = Argon2id(пароль, соль), и этим ключом напрямую зашифрованы
   все базы (Current, Catalog, каждый Archive).
   - Вариант A (рекомендуется): обёртка ключа. Действующий MasterKey становится ключом данных и не
     меняется; `storage-crypto.json` версии 2 хранит его зашифрованным ключом из пароля
     (AES-256-GCM, в AAD — `StorageId` и версия формата). Существующее хранилище переходит на v2 при
     разблокировке. Смена пароля — одна атомарная запись метаданных, базы не трогаются, операция
     мгновенная. Ограничение: если утёк старый пароль вместе со старыми метаданными и копией баз,
     смена пароля от этого не защищает — ключ данных прежний.
   - Вариант B: полная смена ключа — перешифровать все базы новым ключом (`PRAGMA rekey`). Защищает и
     от утечки старого пароля, но это многофайловая durable-операция уровня Archive Rotation
     (смешанное состояние ключей при сбое, время пропорционально объёму архивов).
   - Вариант C: A сейчас, B позже отдельным действием «Сменить ключ шифрования».
2. **Потеря или повреждение `storage-crypto.json`.** Без соли и параметров KDF ключ не вывести даже
   при правильном пароле — вся история теряется. Файл не секретен (соль, параметры, проверочное
   значение). Рекомендуется вторая копия внутри хранилища, которая пишется вместе с основной; при
   повреждении основной используется копия, если её проверочное значение подтверждает пароль, и
   основная восстанавливается из неё.
3. **Потеря `current.db`.** Current хранит policy, группы, identity приложений и свежую историю;
   Catalog источником истины не является. Сейчас — отказ с сообщением. **Решение пользователя:**
   явное действие «Начать текущую базу заново» с подтверждением — новый Current с тем же
   `StorageId` и ключом, архивы остаются в журнале после перестроения Catalog, правила и группы
   настраиваются заново. Не реализовано.
4. **Карантин Catalog.** `Current/CatalogQuarantine` копит прежние Catalog после каждой замены.
   **Решение пользователя:** копии удаляются по тому же сроку, что и корзина внешних файлов
