...
**Решение пользователя (2026-09-22): explicit merge/rebind workflow с иерархией родитель/ребёнок и
ретроактивной очисткой.** Не реализовано. Заменяет собой более простую «плоскую» модель merge/rebind,
изначально намеченную в первой версии этого пункта — вместо простого alias-присоединения вводится
явная группа приложений с родителем и детьми. Полная спецификация — в `APPLICATION_IDENTITY.md`
раздел «9. Merge/rebind identity, группы приложений и ретроактивная очистка» (там же обновлён список
запрещённых эвристик и registry contract). Здесь — только продуктовая суть решения:

- ещё не настроенное (без персональной политики) приложение работает по глобальной политике; в
  журнале такие записи явно помечены «нет индивидуальных настроек»;
- пользователь получает из журнала список приложений без персональной политики и задаёт её;
- **ретроактивная очистка** запускается ровно в двух точках — первое назначение персональной
  политики ранее ненастроенному приложению, и merge (объединение приложений в группу). Редактирование
  уже существующей персональной политики и редактирование глобальной политики очистку **не**
  запускают — они, как и раньше, действуют только на будущее;
- очистка идёт на уровне **конкретного представления формата** внутри записи (например, элемент
  буфера с Text+HTML при запрете HTML теряет только HTML-представление, Text остаётся); то же
  правило распространяется на превышение размера — слишком большое для нового лимита представление
  удаляется само по себе. Если после этого в записи не остаётся ни одного представления — удаляется
  вся запись целиком, включая связанные внешние файлы;
- перед очисткой обязательно подтверждение пользователя; экран подтверждения показывает итоговые
  цифры (сколько представлений/записей будет удалено, сколько записей исчезнет целиком), с
  возможностью по запросу развернуть список конкретных затронутых записей;
- merge — явное действие пользователя, всегда направленное: **дочернее** приложение присоединяется к
  **родительскому**. Итоговая политика группы — это всегда политика родителя. Если у ребёнка была
  своя персональная политика — экран merge показывает обе политики и по умолчанию действует политика
  родителя; пользователь может до подтверждения скопировать в неё отдельные правила из политики
  ребёнка, но после merge в группе действует ровно одна политика, не «своя для каждого ребёнка»;
- группа — не отдельная сущность: identity родителя (`ApplicationId`) исполняет роль identity группы,
  у ребёнка появляется nullable `ParentApplicationId`. Группа **плоская** — родитель не может сам быть
  чьим-то ребёнком (не более одного уровня вложенности);
- **split**: пользователь может выделить ребёнка обратно в отдельное приложение. Ребёнок получает
  **тот же `ApplicationId`, что был у него до merge** (merge не уничтожает identity ребёнка, только
  помечает её как состоящую в группе) — благодаря этому при split не нужен новый identity resolution.
  Clipensk спрашивает, переносить ли в отделяемую identity записи, накопленные у родителя за время
  пребывания в группе. Записи, безвозвратно вычищенные ещё на этапе merge, split не восстанавливает —
  это уже случилось;
- журнал хранит точное происхождение каждой записи (родитель или конкретный ребёнок) даже после
  merge — разрез аналитики по бывшим детям не теряется. Фильтр в журнале работает по группе (identity
  родителя + все текущие дети) с возможностью включать/выключать в фильтре отдельных членов группы;
- технически удаление внешних файлов (изображения/custom binary) при очистке переиспользует уже
  существующий путь `Files → Trash → retention` (тот же, что уже работает для last-reference
  cleanup), а не отдельный новый механизм.

Автоматические эвристики merge по-прежнему запрещены (см. `APPLICATION_IDENTITY.md` §5) — merge
только по явному действию пользователя, без silent equivalence по имени/hash/издателю/пути.

Реализация — отдельный заход, по архитектурной сложности и цене ошибки (ретроактивное безвозвратное
удаление зашифрованной истории и внешних файлов, включая Archive) сопоставима с Archive
Rotation/Split.
