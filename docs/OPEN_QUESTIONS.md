...
- **Escape и «клик мимо»/авто-скрытие журнала как самостоятельные триггеры скрытия окна.** В коде
  сегодня нет ни обработки клавиши Escape, ни auto-hide по потере активации: единственный способ
  скрыть окно — кнопка-крестик (`OnAppWindowClosing`). Restoration уже сработает для любого нового
  триггера, который в будущем тоже будет скрывать окно через `sender.Hide()` в этом же обработчике,
  но добавление именно Escape/click-away/auto-hide как новых UX-триггеров — отдельная, ещё не
  начатая задача, требующая явного продуктового решения о точном поведении (например, не скрывать
  окно, пока открыт системный диалог выбора файла, иначе загрузка файла перевода будет ошибочно
  прятать окно при потере активации).
