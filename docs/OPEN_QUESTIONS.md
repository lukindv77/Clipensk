...
раздел «9. Merge/rebind identity, группы приложений и ретроактивная очистка»; список запрещённых
автоматических эвристик (§5 там же) остаётся в силе без изменений. Правило очистки истории в
`REQUIREMENTS.md` §18 обновлено под это решение. Здесь — только продуктовая суть решения:
...
- **ретроактивная очистка** запускается ровно в двух точках — первое назначение персональной
  ...
  запускают и действуют только на будущее. Это **меняет** прежнее поведение: до решения любое
  отключение параметра сбора чистило историю (`REQUIREMENTS.md` §18, уже реализовано). Первоначально
  этот вариант был выбран по ошибочному описанию ассистента, будто правка политики и так действует
  только на будущее; после исправления вводной пользователь 2026-09-22 подтвердил его повторно;
- очистка идёт на уровне **конкретного представления формата** внутри записи (например, элемент
  буфера с Text+HTML при запрете HTML теряет только HTML-представление, Text остаётся), только по
  правилу Allow/Deny — лимит размера при очистке **не** учитывается, он действует только при
  захвате. Если после этого в записи не остаётся ни одного представления — удаляется вся запись
  целиком (для любой очистки по политике; раньше оставались пустые записи);
- очистка охватывает Current и **Archive полностью**, включая встроенные данные (текст, HTML, RTF,
  ссылки, списки файлов), а не только ссылки на внешние файлы, как было раньше; внешний файл уходит
  в Trash, когда на него не осталось ссылок;
- перед любой очисткой обязательно подтверждение пользователя; экран подтверждения показывает
...
- **split**: пользователь может выделить ребёнка обратно в отдельное приложение. Ребёнок получает
  **тот же `ApplicationId`, что был у него до merge** (merge не уничтожает identity ребёнка, только
  ...
  ребёнок и которые сейчас принадлежат группе (и принесённые при merge, и накопленные позже); при
  отказе они остаются в группе родителя, сохраняя происхождение. Записи, безвозвратно вычищенные ещё
  на этапе merge, split не восстанавливает — это уже случилось;
...
- технически удаление внешних файлов (изображения/custom binary) при очистке переиспользует уже
  ...
  cleanup), а не отдельный новый механизм. Файл остаётся в Trash (незашифрованным) до истечения
  обычного срока хранения, по умолчанию 30 дней, — это выбор пользователя от 2026-09-22 в пользу
  запаса на случай ошибки, сделанный после исправления ошибочного описания ассистента («неотличимо
  от немедленного удаления»).

Техническая реализация, модель данных (Current v11, `ApplicationGroupMember`), durable-фазы и этапы —
`APPLICATION_GROUP_PROTOCOL.md`.
