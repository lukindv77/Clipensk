...
Последний промотированный код в main: `4758546` — §I.9 (перенос хранилища и `settings.json` рядом
...
34. Перенос хранилища и `settings.json` рядом с программой (`OPEN_QUESTIONS.md` §12, решения
    ...
    - `DataRootRelocationService` (Infrastructure): побайтная копия дерева каталога данных из
      ...
      файлов источника, которые цель хранит байт в байт (метаданные ключа — последними);
...
Storage 681, Core 315, Infrastructure 98 — все зелёные локально и в CI. Ручной smoke групп и
переноса хранилища на Windows — `UNVERIFIED`.
...
10. Manual production WinUI/storage smoke evidence (остаётся `UNVERIFIED`). Покрывает возврат в буфер
    ...
    **tray-иконку/автозапуск**, **single-instance/ACL каталога данных** и **группы приложений**
    ...
    без ACL) и **`settings.json` рядом с программой** (папка без права записи) — весь этот
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — итоговый коммит §I.9 поверх `4758546`. §I.9 (перенос хранилища) и §I.11 (группы приложений) завершены; открытым остаётся ручной smoke на Windows (§I.10) — его может выполнить только пользователь; новые задачи — по решению пользователя.
