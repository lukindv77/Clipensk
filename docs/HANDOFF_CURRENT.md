...
Последний промотированный main: `f88d524` — этапы 1–5 из 13 реализации §11 (группы приложений и
очистка истории), exact-SHA evidence каждого — в `docs/APPLICATION_GROUP_PROTOCOL.md` §10 Accepted.
...
Storage 601, Core 282, Infrastructure 75 — все зелёные локально и в CI.
...
11. **Merge/rebind identity отслеживаемых приложений** (`OPEN_QUESTIONS.md` §11,
    `APPLICATION_IDENTITY.md` §9) — **продуктовое решение принято 2026-09-22; реализация идёт**:
    этапы 1–5 из 13 (`docs/APPLICATION_GROUP_PROTOCOL.md` §10) в `main` — Current v11 с таблицей
    `ApplicationGroupMember`, эффективная policy через корень группы, правка глобальной и уже
    заданной policy без очистки истории, движок `ApplicationHistoryPurge` (в App ещё не подключён).
    Следующий — этап 6 (предпросмотр очистки + фильтр журнала по формату).
    После двух предыдущих раз, когда пользователь отвечал не по теме §11 (путал с собственным
    single-instance/portability-поведением Clipensk, см. §12/пункт 9 выше), ассистент вынес §11 в
    отдельное развёрнутое обсуждение и последовательно закрыл все развилки через явные вопросы
    пользователю. Решение: явный направленный merge (`Child` → `Parent`) с плоской группой
    (группа = `ApplicationId` корня, членство — отдельная таблица `ApplicationGroupMember`; не более
    одного уровня вложенности); ретроактивная безвозвратная очистка истории
    запускается только при первом назначении персональной policy и при merge; очистка — на уровне
    отдельного представления формата внутри записи, только по Allow/Deny (без учёта `MaxBytes`), в
    Current и в Archive полностью (включая встроенные данные), с удалением всей записи, если не
    осталось ни одного представления; обязательное подтверждение с итоговыми цифрами перед
    очисткой; split возвращает ребёнку тот же `ApplicationId`, что был до merge, но не
    восстанавливает уже выполненную очистку; журнал хранит провенанс записей по родителю/ребёнку и
    фильтрует по группе с переключением отдельных членов.
    **Важно — это меняет ранее реализованное поведение** (`REQUIREMENTS.md` §18 обновлён): раньше
    `ProtectedCurrentPolicyMaintenanceService` и `ProtectedCurrentApplicationPolicyMaintenanceService`
    чистили историю при **любой** правке глобальной/индивидуальной policy, из Archive удаляли только
    ссылки на внешние файлы и оставляли пустые записи. С этапа 4 правка глобальной и уже заданной
    policy историю не чистит; первое назначение до этапа 7 ещё идёт прежним путём. Первоначально пользователь выбрал это по ошибочному
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — потомок `f88d524`. Продолжение §I.11 — со следующего непринятого этапа `docs/APPLICATION_GROUP_PROTOCOL.md` §10.
...
режим «вставить как plain text», и дефолты/KB-редактирование числовых лимитов форматов). MSIX
(бывший §I.7) снят решением пользователя от 2026-09-21 — задачи по нему больше нет. Из оставшегося:
...
- пункт §I.11 (merge/rebind identity + группы приложений, `APPLICATION_IDENTITY.md` §9) — решение
  принято 2026-09-22, но реализация — **Opus, высокая сложность**, по той же причине: ретроактивное
  безвозвратное удаление зашифрованной истории Current+Archive и внешних файлов, координация через
  protected-session/mutation-lease, по цене ошибки аналог Archive Rotation/Split;
...
Итог: оба оставшихся содержательных пункта (§I.9 перенос данных, §I.11 merge/rebind identity)
требуют Opus/высокой сложности по цене ошибки — на Sonnet 5 из содержательного остаётся только
ручная проверка на Windows либо продуктовые уточнения от пользователя, если появятся новые.
