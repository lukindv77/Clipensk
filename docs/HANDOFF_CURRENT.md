...
Последний промотированный main: `4851e07` (потомок `bbef1326070f126642417835762a7929112e2289`
через семь последовательных fast-forward slice за этот заход).
...
- `4851e07` (дефолты числовых лимитов форматов + редактирование лимитов в целых килобайтах,
  **текущий HEAD**): Build run `35567755745` — **SUCCESS** (dispatch на ветке `feat/format-limits-kilobytes`
...
11. **Merge/rebind identity отслеживаемых приложений** (`OPEN_QUESTIONS.md` §11,
    `APPLICATION_IDENTITY.md` §9) — **продуктовое решение принято 2026-09-22** (не реализовано).
    ...
    пользователю. Решение: явный направленный merge (`Child` → `Parent`) с иерархией
    родитель/ребёнок через nullable `ParentApplicationId` (группа = identity родителя, не отдельная
    сущность; не более одного уровня вложенности); ретроактивная безвозвратная очистка истории
    ...
    **Важно — это меняет уже реализованное поведение** (`REQUIREMENTS.md` §18 обновлён): сейчас
    `ProtectedCurrentPolicyMaintenanceService` и `ProtectedCurrentApplicationPolicyMaintenanceService`
    чистят историю при **любой** правке глобальной/индивидуальной policy, из Archive удаляют только
    ссылки на внешние файлы и оставляют пустые записи. По решению правка уже настроенной и глобальной
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — потомок `4851e07`.
