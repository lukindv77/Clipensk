...
опубликованного runtime) — **SUCCESS**; поверх — коммит handoff (плюс уточнение одной строки
отказа смены пароля), его Build на ветке — **SUCCESS** до промоушена. Exact-main evidence этого
коммита записывается следующим коммитом.
...
Storage 721, Core 327, Infrastructure 143 — все зелёные локально. Ручной smoke групп, переноса
...
12. **Криптография: решённые пункты** (`OPEN_QUESTIONS.md` §4, решения 2026-09-23) — реализованы
    ...
    §E пункты 36–38, ревизия — пункт 39. Открыт только выбор резервного копирования
    (`OPEN_QUESTIONS.md` §5) — решение пользователя.
...
10. Manual production WinUI/storage smoke evidence (остаётся `UNVERIFIED`). Покрывает возврат в буфер
    ...
    **tray-иконку/автозапуск**, **single-instance/ACL каталога данных** и **группы приложений**
    ...
    обрыв посреди переключения, разблокировка старым и новым паролем после обрыва) — весь этот
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — коммит handoff поверх `7bcc6d0` (ревизия смены пароля, §E пункт 39; под ним `63b72e7` — анализ резервного копирования в `OPEN_QUESTIONS.md` §5, `6718a03`, `771db40`, `763308a`). §I.9 (перенос хранилища), §I.11 (группы приложений) и модель ключа завершены; решённые крипто-пункты §I.12 реализованы (§E пункты 36–39). Дальше — резервное копирование по выбору пользователя (`OPEN_QUESTIONS.md` §5) и ручной smoke на Windows (§I.10), который может выполнить только пользователь.
...
- **резервное копирование** (`OPEN_QUESTIONS.md` §5), после выбора варианта пользователем —
  **Opus, высокая сложность**: копия живого зашифрованного хранилища (под арендой мутаций или при
  блокировке) и восстановление «Открыть хранилище из папки…» по цене ошибки аналогичны переносу
  хранилища;
- чистая проверка CI и промоушен уже готового кода — Sonnet 5, средняя сложность;
- **ручная проверка на Windows** (§I.10) — не задача для модели: весь WinUI/Win32-код (tray,
  автозапуск, single-instance/ACL, группы, перенос хранилища, разблокировка по заголовкам баз,
  «Начать текущую базу заново», смена пароля) проверен CI только на уровне компиляции.
