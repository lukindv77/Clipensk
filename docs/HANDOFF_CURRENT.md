...
Последний промотированный код в main: `ac7e9cb` — §11 (группы приложений и очистка истории)
реализован: этапы 1–6 по версии 1 протокола, 7–14 по версии 2 (группа — отдельная сущность,
решение 2026-09-23) и исправление фильтра групп журнала `ac7e9cb`. Exact-SHA evidence каждого
коммита и exact-main evidence промоушена `0de023c..c4886bf` — в `docs/APPLICATION_GROUP_PROTOCOL.md`
§10 Accepted; поверх `ac7e9cb` — docs-коммит этапа 15 (этот файл и связанные документы).
...
Storage 681, Core 304, Infrastructure 75 — все зелёные локально и в CI. Ручной smoke групп на
Windows — `UNVERIFIED`.
...
10. Manual production WinUI/storage smoke evidence (остаётся `UNVERIFIED`). Покрывает возврат в буфер
    ...
    **tray-иконку/автозапуск**, **single-instance/ACL каталога данных** и **группы приложений**
    (перенос с очисткой и экран подтверждения, журнал по группе, управление группами) — весь этот
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — docs-коммит этапа 15 поверх `ac7e9cb`. §I.11 (группы приложений) завершён; следующая работа — §I.9 (перенос настроек и базы при смене пути) по решению пользователя либо ручной smoke (§I.10).
