...
Storage 698, Core 327, Infrastructure 114 — все зелёные локально. Ручной smoke групп, переноса
...
- Смена пароля не реализована: по решению 2026-09-23 это перешифровка всех баз новым MasterKey (соль прежняя), нужен отдельный отказоустойчивый протокол (`CRYPTOGRAPHY.md` §12).
...
12. **Криптография: оставшиеся решённые пункты** (`OPEN_QUESTIONS.md` §4, решения 2026-09-23) — не
    реализованы:
    - смена пароля = перешифровка всех баз новым MasterKey, соль прежняя (`CRYPTOGRAPHY.md` §12);
    Удаление копий `Current/CatalogQuarantine` по сроку корзины и «Начать текущую базу заново» —
    реализованы (§E пункты 36, 37).
...
10. Manual production WinUI/storage smoke evidence (остаётся `UNVERIFIED`). Покрывает возврат в буфер
    ...
    **tray-иконку/автозапуск**, **single-instance/ACL каталога данных** и **группы приложений**
    ...
    удалённый `current.db` и «Начать текущую базу заново») — весь этот
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — коммит handoff поверх `68ec635` (ключ хранилища без `storage-crypto.json`, §E пункт 35). §I.9 (перенос хранилища), §I.11 (группы приложений) и модель ключа завершены; открыт решённый, но не реализованный крипто-пункт §I.12 — смена пароля (срок карантина Catalog и «Начать текущую базу заново» реализованы — §E пункты 36, 37) и ручной smoke на Windows (§I.10) — его может выполнить только пользователь.
