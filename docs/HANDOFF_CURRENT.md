...
7. **MSIX-упаковка** в дополнение к unpackaged (`OPEN_QUESTIONS.md` §3). Требует отдельного
   согласования с пользователем перед началом: затрагивает `.github/**` и схему доставки
   `sqlcipher.dll`.
...
и режим «вставить как plain text»). Из оставшегося:

- пункт §I.7 (MSIX) — **Sonnet 5, высокая сложность** по объёму работы, но затрагивает `.github/**`
  и схему доставки `sqlcipher.dll`, поэтому требует отдельного явного согласования с пользователем
  до начала (что именно должно измениться в workflow, версионирование installer'а и т.д.) —
  рекомендация для самого шага согласования: любая модель, обсуждение не требует кода;
...
Итог: если у пользователя нет готового ответа на MSIX-вопросы, числовых лимитов форматов или
решения по §11, следующий шаг с кодом на Sonnet 5 отсутствует — остаются либо Opus-уровня перенос
данных (§I.9, но пользователь просил отдельный заход), либо ручная проверка на Windows, либо
