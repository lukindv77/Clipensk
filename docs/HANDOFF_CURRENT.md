...
11. **Merge/rebind identity отслеживаемых приложений** (`OPEN_QUESTIONS.md` §11,
    ...
    запускается только при первом назначении персональной policy и при merge (не при обычном
    редактировании policy); очистка — на уровне отдельного представления формата внутри записи, с
    удалением всей записи только если не осталось ни одного представления; обязательное
    подтверждение с итоговыми цифрами перед очисткой; split возвращает ребёнку тот же
    `ApplicationId`, что был до merge, но не восстанавливает уже выполненную очистку; журнал хранит
    провенанс записей по родителю/ребёнку и фильтрует по группе с переключением отдельных членов.
    ...
    внешних файлов) аналог Archive Rotation/Split, отдельный заход.
