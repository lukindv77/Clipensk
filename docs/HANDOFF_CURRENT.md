...
Последний промотированный код в main: `a4983eb` — этапы 1–6 реализации §11 (группы приложений и
очистка истории), exact-SHA evidence каждого — в `docs/APPLICATION_GROUP_PROTOCOL.md` §10 Accepted.
2026-09-23 модель групп уточнена (группа — отдельная сущность); протокол переписан как версия 2.
...
11. **Группы приложений и очистка истории** (`OPEN_QUESTIONS.md` §11, `APPLICATION_IDENTITY.md` §9,
    `docs/APPLICATION_GROUP_PROTOCOL.md` версии 2) — **реализуется**.
    ...
    В `main` уже есть этапы 1–6 по версии 1: v11 `ApplicationGroupMember`, разрешение через корень,
    publish-only правка policy, движок очистки, предпросмотр и `ClipboardHistoryFilter`. Что из
    этого переиспользуется, а что заменяется, — §11 протокола. Storage-часть подтверждённого старта
    и координатор лежат в ветке `feat/first-assignment-confirmation` (`24b2e23`, в `main` не
    продвигалась) и адаптируются в этапе 11.
    Следующий — этап 8 протокола v2 (Core-модель групп), затем этап 9 (Current v12 и миграция).
    Реализация — **Opus, высокая сложность**: ретроактивное безвозвратное удаление зашифрованной
    истории и миграция схемы по цене ошибки — аналог Archive Rotation/Split.
...
2. Fresh-check `origin/main`; ожидаемое значение на момент checkpoint — потомок `a4983eb`. Продолжение §I.11 — со следующего непринятого этапа `docs/APPLICATION_GROUP_PROTOCOL.md` §10 (версия 2).
...
- пункт §I.11 (группы приложений, `APPLICATION_IDENTITY.md` §9) — решение принято 2026-09-22 и
  уточнено 2026-09-23, реализация идёт — **Opus, высокая сложность**, по той же причине: ретроактивное
...
Итог: оба оставшихся содержательных пункта (§I.9 перенос данных, §I.11 группы приложений)
