...
- `SupportedOSPlatformVersion`: `10.0.22000.0` — минимальная официально поддерживаемая версия ОС; продуктовая поддержка — **только Windows 11** (Windows 10 вне поддержки, решение пользователя от 2026-09-20, `OPEN_QUESTIONS.md` §2); на более старой сборке (ниже 22000) Clipensk при старте предупреждает, что версия не поддерживается, и продолжает работу (`SupportedWindowsVersion`, решение 2026-09-23);
- текущий development-host собирает unpackaged x64; финальная и единственная схема распространения — unpackaged/portable (решение пользователя от 2026-09-21, `OPEN_QUESTIONS.md` §3, заменяет более раннее решение о MSIX от 2026-09-20 — от MSIX отказались).
...
Пароль никогда не сохраняется. Отдельного файла криптографических метаданных нет: соль Argon2id
(первый байт — номер KDF-профиля) лежит открыто в первых 16 байтах каждой базы, и для открытия
хранилища достаточно любой уцелевшей базы и пароля. Пароль проверяется пробным открытием базы
(`CRYPTOGRAPHY.md` §3, §4).
