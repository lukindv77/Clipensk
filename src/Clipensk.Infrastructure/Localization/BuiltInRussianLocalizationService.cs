...
public sealed class BuiltInRussianLocalizationService : ILocalizationService
{
    private static readonly IReadOnlyDictionary<string, string> Strings = new Dictionary<string, string>(StringComparer.Ordinal)
    {
        ...
        ["CapturePolicy.Title"] = "Первичная настройка правил сбора",
        ...
        ["CapturePolicy.SetupHelp"] = "Это первоначальное сохранение правил и custom-binary mappings. Их последующее изменение пока недоступно. После успешного COMMIT App пересобирает protected capture runtime для текущей разблокированной session.",
        ...
        ["CapturePolicy.State.Configured"] = "Первоначальные правила сохранены. Пока хранилище разблокировано, resident capture runtime использует эту policy; последующее изменение policy ещё не реализовано.",
