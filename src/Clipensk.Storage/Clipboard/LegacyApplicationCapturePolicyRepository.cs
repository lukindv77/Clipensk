...
/// <summary>
/// Reads and writes the legacy per-application capture policy tables, which Current v12 keeps only
/// for a pending legacy <c>ApplicationPolicyMaintenance</c> operation
/// (<c>docs/APPLICATION_GROUP_PROTOCOL.md</c> §2.2). Capture and settings never use them; this type
/// exists to exercise and verify that legacy continuation.
/// </summary>
internal sealed class LegacyApplicationCapturePolicyRepository
{
    private readonly ProtectedStorageSessionLease _session;
    private readonly IKeyedSqliteConnectionFactory _connectionFactory;
    private readonly string _currentDatabasePath;

    public LegacyApplicationCapturePolicyRepository(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using SqliteConnection connection = OpenValidatedCurrent(SqliteOpenMode.ReadOnly, token, out _);
        using SqliteTransaction transaction = connection.BeginTransaction(deferred: true);
        ClipboardCapturePolicy? policy = ReadApplicationPolicy(connection, transaction, applicationId, token);
        token.ThrowIfCancellationRequested();
