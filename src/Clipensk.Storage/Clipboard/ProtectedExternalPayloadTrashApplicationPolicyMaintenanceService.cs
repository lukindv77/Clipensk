...
public sealed record ExternalPayloadTrashApplicationPolicyMaintenanceResult(
    PendingPolicyMaintenanceOperation Operation,
    int CollectedFileCount,
    IReadOnlyList<string> TrashRelativePaths,
    bool WasAlreadyCompleted);

internal enum ExternalPayloadTrashApplicationPolicyMaintenanceCheckpoint
{
    CollectionCompleted,
    BeforeMarkerCommit,
    AfterMarkerCommit,
...
/// <summary>
/// Continues one committed application capture-policy maintenance operation by collecting only
/// storage-wide history-unreferenced managed external payload objects into Trash and then durably
/// marking only that application's external Trash continuation phase completed. Final completion
/// and pending-marker removal remain a later resumable phase.
/// </summary>
public sealed class ProtectedExternalPayloadTrashApplicationPolicyMaintenanceService
{
    private readonly ProtectedStorageSessionLease _session;
    private readonly IKeyedSqliteConnectionFactory _connectionFactory;
    private readonly string _currentDatabasePath;
    private readonly Action<ExternalPayloadTrashApplicationPolicyMaintenanceCheckpoint>? _checkpoint;

    public ProtectedExternalPayloadTrashApplicationPolicyMaintenanceService(
    ...
    internal ProtectedExternalPayloadTrashApplicationPolicyMaintenanceService(
        ProtectedStorageSessionLease session,
        IKeyedSqliteConnectionFactory? connectionFactory,
        Action<ExternalPayloadTrashApplicationPolicyMaintenanceCheckpoint>? checkpoint)
    {
    ...
    public async Task<ExternalPayloadTrashApplicationPolicyMaintenanceResult> ApplyAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (snapshot.State.ExternalTrashCollection ==
            ApplicationPolicyMaintenanceState.Completed)
        {
            token.ThrowIfCancellationRequested();
            return new ExternalPayloadTrashApplicationPolicyMaintenanceResult(
                snapshot.Operation,
        ...
        if (snapshot.State.CatalogRebuild != ApplicationPolicyMaintenanceState.Completed)
        {
            throw new InvalidOperationException(
                "Application external Trash maintenance requires completed Catalog rebuild.");
        }
        ...
        // The collector is intentionally storage-wide because file liveness is storage-wide. Each
        // successful move/delete is independently durable, so cancellation before marker commit is
        // retryable: already-collected objects are simply absent from Files on the next pass.
        _checkpoint?.Invoke(
        ...
        // No cancellation check after marker COMMIT. Once this application continuation phase is
        // durably completed, late cancellation cannot demote the successful result.
        return new ExternalPayloadTrashApplicationPolicyMaintenanceResult(
            completed,
    ...
    private CurrentSnapshot ReadCurrentSnapshot(CancellationToken token)
    {
        ...
        PendingPolicyMaintenanceOperation operation =
            SqlitePendingPolicyMaintenanceRepository.ReadInTransaction(
                ...
                token)
            ?? throw new InvalidOperationException(
                "Application external Trash maintenance requires a pending application policy-maintenance operation.");
        if (!string.Equals(
        ...
        ClipboardCapturePolicy applicationPolicy =
            ...
            ?? throw new InvalidDataException(
                "Application external Trash maintenance requires the committed target application policy.");
        if (!string.Equals(
        ...
        CancellationToken token)
    {
        ...
        PendingPolicyMaintenanceOperation current =
            SqlitePendingPolicyMaintenanceRepository.ReadInTransaction(
                ...
                token)
            ?? throw new InvalidOperationException(
                "The pending application policy-maintenance operation disappeared before external Trash completion.");
        ...
        {
            throw new InvalidOperationException(
                "The pending policy-maintenance operation changed before application external Trash completion.");
        }
        ...
        {
            throw new InvalidOperationException(
                "The pending application policy-maintenance target changed before external Trash completion.");
        }

        DurableApplicationId currentApplicationId = ParseApplicationId(currentState.ApplicationId);
        if (!string.Equals(
        ...
        {
            throw new InvalidOperationException(
                "The pending application policy-maintenance application changed before external Trash completion.");
        }
        ValidateApplicationIdentity(connection, transaction, currentApplicationId, token);

        ClipboardCapturePolicy applicationPolicy =
            ReadApplicationPolicy(connection, transaction, currentApplicationId, token)
            ?? throw new InvalidDataException(
                "Application external Trash completion requires the committed target application policy.");
        if (!string.Equals(
                ApplicationPolicyMaintenanceStateCodec.ComputePolicyFingerprint(applicationPolicy),
                currentState.PolicyFingerprint,
                StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                "The application capture policy changed before external Trash completion.");
        }

        if (currentState.CatalogRebuild != ApplicationPolicyMaintenanceState.Completed)
        {
            throw new InvalidOperationException(
                "Application external Trash completion requires completed Catalog rebuild.");
        }

        if (currentState.ExternalTrashCollection == ApplicationPolicyMaintenanceState.Completed)
        {
            token.ThrowIfCancellationRequested();
            return current;
        }
        ...
        ApplicationPolicyMaintenanceState completedState =
            currentState with
            {
                ExternalTrashCollection = ApplicationPolicyMaintenanceState.Completed,
            };
        ...
        _checkpoint?.Invoke(
            ExternalPayloadTrashApplicationPolicyMaintenanceCheckpoint.BeforeMarkerCommit);

        // Final cancellation boundary. The application policy is revalidated while this mutation
        // lease is held, so supported policy writers cannot race the durable marker publication.
        token.ThrowIfCancellationRequested();
        transaction.Commit();
    ...
    private void ValidateCurrentIdentity(SqliteConnection connection)
    {
        using (SqliteCommand identity = connection.CreateCommand())
        {
            ...
            {
                throw new InvalidDataException(
                    "Application external Trash maintenance requires the exact Current v7 identity.");
            }
        }
        ...
        {
            throw new InvalidDataException(
                "Application external Trash maintenance requires matching Current v7 user_version.");
        }
    }
        ...
        CancellationToken token)
    {
        ...
        {
            throw new InvalidDataException(
                "Application external Trash maintenance requires the exact target application identity.");
        }
    }
    ...
    private static void ValidateForeignKeys(SqliteConnection connection)
    {
        ...
        {
            throw new InvalidDataException(
                "Application external Trash maintenance found invalid Current foreign-key data.");
        }
    }
