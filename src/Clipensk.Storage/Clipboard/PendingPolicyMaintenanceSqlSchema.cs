...
internal static class PendingPolicyMaintenanceSqlSchema
{
    ...
    public static void CreateTable(SqliteConnection connection, SqliteTransaction transaction)
    {
        ...
        command.CommandText = """
            CREATE TABLE PendingPolicyMaintenance (
                SingletonId INTEGER NOT NULL PRIMARY KEY CHECK (SingletonId = 1),
                OperationId TEXT NOT NULL UNIQUE CHECK (length(OperationId) > 0),
                OperationKind TEXT NOT NULL CHECK (length(OperationKind) > 0),
                StateJson TEXT NOT NULL CHECK (length(StateJson) > 0),
                CreatedAtUtc TEXT NOT NULL CHECK (length(CreatedAtUtc) > 0),
                UpdatedAtUtc TEXT NOT NULL CHECK (length(UpdatedAtUtc) > 0)
            );
            """;
    ...
    public static void ValidateTable(SqliteConnection connection)
    {
        ...
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText = "PRAGMA table_info('PendingPolicyMaintenance');";
        ...
        [
            ...
            new("OperationKind", "TEXT", NotNull: true, PrimaryKeyOrder: 0),
            new("StateJson", "TEXT", NotNull: true, PrimaryKeyOrder: 0),
            new("CreatedAtUtc", "TEXT", NotNull: true, PrimaryKeyOrder: 0),
            new("UpdatedAtUtc", "TEXT", NotNull: true, PrimaryKeyOrder: 0),
        ];
        ...
        while (reader.Read())
        {
            ...
            {
                throw new InvalidDataException("PendingPolicyMaintenance contains unexpected columns.");
            }
            ...
            {
                throw new InvalidDataException("PendingPolicyMaintenance column contract is invalid.");
            }
        }

        if (index != expected.Length)
        {
            throw new InvalidDataException("PendingPolicyMaintenance is missing required columns.");
        }
    }
