...
internal sealed class SqliteArchiveClipboardHistoryRepository
{
    private readonly ProtectedStorageSessionLease _session;
    private readonly IKeyedSqliteConnectionFactory _connectionFactory;
    private readonly ArchiveSegmentDescriptor _segment;
    private readonly ArchiveFileName _archiveFileName;
    private readonly ProtectedArchiveDatabaseService _archiveService;
    private readonly string _archiveDatabasePath;
    private readonly string _filesRootPath;

    public SqliteArchiveClipboardHistoryRepository(
        ProtectedStorageSessionLease session,
        ArchiveSegmentDescriptor segment,
        IKeyedSqliteConnectionFactory? connectionFactory = null)
    {
        ...
        if (segment.DatabaseId == Guid.Empty)
        {
            throw new ArgumentException("Archive segment DatabaseId cannot be empty.", nameof(segment));
        }
        if (!ArchiveFileName.TryParse(segment.FileName, out ArchiveFileName archiveFileName) ||
            !string.Equals(segment.FileName, archiveFileName.FileName, StringComparison.Ordinal))
        {
            throw new ArgumentException(
                "Archive segment FileName must be canonical.",
                nameof(segment));
        }
        ...
        _archiveDatabasePath = Path.Combine(root, "Archive", archiveFileName.FileName);
        _filesRootPath = Path.Combine(root, "Files") + Path.DirectorySeparatorChar;
    }

    public Task<IReadOnlyList<ClipboardHistoryEntry>> ReadAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (before is not null && before.Period != period)
        {
            throw new ArgumentException(
        ...
        CancellationToken cancellationToken)
    {
        ...
        DatabaseIdentity beforeIdentity = await _archiveService
            .ValidateAsync(_archiveFileName, token)
        ...
        DatabaseIdentity afterIdentity = await _archiveService
            .ValidateAsync(_archiveFileName, token)
            .ConfigureAwait(false);
        ValidateIdentityMatchesSegment(afterIdentity);

        token.ThrowIfCancellationRequested();
        return entries;
    }

    private IReadOnlyList<ClipboardHistoryEntry> ReadDatabaseCore(
        JournalDateRange period,
        int limit,
        ClipboardHistoryCursor? before,
        CancellationToken token)
    {
        ...
        command.Parameters.AddWithValue(
            "$endDate",
            period.EndDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
        ...
        while (reader.Read())
        {
            ...
            if (!string.Equals(storedEventId, eventId.ToString("D"), StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    "History EventId must retain the canonical lowercase form used by the sink and cursor.");
            }
            ...
            if (!reader.IsDBNull(9))
            {
                ...
                {
                    throw new InvalidDataException(
                        "History payload order must be contiguous and start at zero.");
                }
    ...
    private SqliteConnection OpenValidatedArchive(CancellationToken token)
    {
        token.ThrowIfCancellationRequested();
        ...
        try
        {
            using (SqliteCommand identity = connection.CreateCommand())
            {
                ...
                if (!reader.Read() ||
                    ...
                    databaseId != _segment.DatabaseId ||
                    !string.Equals(
                        reader.GetString(2),
                        DatabaseRole.Archive.ToString(),
                        StringComparison.Ordinal) ||
                    reader.GetInt32(3) != ProtectedArchiveDatabaseService.ArchiveSchemaVersion ||
                    !TryReadCoverage(reader, out JournalDateRange coverage) ||
                    coverage != _segment.Coverage)
                {
                    throw new InvalidDataException(
                        "Archive history reader identity does not match the planned segment.");
                }
            }
            ...
            {
                throw new InvalidDataException(
                    "Archive history user_version does not match the archive schema version.");
            }
    ...
    private void ValidateIdentityMatchesSegment(DatabaseIdentity identity)
    {
        if (identity.StorageId != _session.StorageId ||
            identity.DatabaseId != _segment.DatabaseId ||
            identity.Role != DatabaseRole.Archive ||
            identity.SchemaVersion != ProtectedArchiveDatabaseService.ArchiveSchemaVersion ||
            identity.CoverageStartDate != _segment.Coverage.StartDate ||
            identity.CoverageEndDate != _segment.Coverage.EndDate)
        {
            throw new InvalidDataException(
                "Archive identity changed or does not match the Catalog query plan.");
        }
    }
        ...
        IReadOnlyList<ClipboardHistoryPayload> payloads)
    {
        ...
        {
            throw new InvalidDataException(
                "History event has invalid persisted time metadata.");
        }
        ...
        {
            throw new InvalidDataException(
                "History event has invalid offset or time-zone metadata.");
        }

        var eventTime = new EventTimeContext(
            new DateTimeOffset(utc).ToOffset(TimeSpan.FromMinutes(offsetMinutes)),
            timeZoneId);
        if (eventTime.CalendarDate != calendarDate)
        {
            throw new InvalidDataException(
                "History event calendar date disagrees with its persisted timestamp and offset.");
        }
        ...
        if (!reader.IsDBNull(6))
        {
            ...
            {
                throw new InvalidDataException(
                    "History source process ID is invalid.");
            }
            source = new ClipboardSourceApplication(
                (uint)processId,
                executablePath,
                aumid);
        }
        ...
        {
            throw new InvalidDataException(
                "History source snapshot is missing its process ID.");
        }
    ...
    private ClipboardHistoryPayload ReadPayload(SqliteDataReader reader)
    {
        ...
        ClipboardHistoryPayloadKind kind = reader.GetString(11) switch
        {
            ...
            _ => throw new InvalidDataException(
                "History contains an unsupported payload kind."),
        };
        ...
        {
            throw new InvalidDataException(
                "History contains invalid payload metadata.");
        }
        ...
        if (kind is ClipboardHistoryPayloadKind.PngImage or ClipboardHistoryPayloadKind.CustomBinary)
        {
            if (inline is not null ||
                ...
                sha is null ||
                sha.Length != 64 ||
                sha.Any(c => c is not (>= '0' and <= '9') and not (>= 'a' and <= 'f')) ||
                string.IsNullOrWhiteSpace(path) ||
                Path.IsPathRooted(path) ||
                !Path.GetFullPath(Path.Combine(_filesRootPath, path))
                    .StartsWith(_filesRootPath, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidDataException(
                    "History contains an invalid external payload reference.");
            }

            external = new ClipboardHistoryExternalReference(
                sha,
                path,
                size!.Value);
        }
        else if (inline is null ||
                 sha is not null ||
                 path is not null ||
                 size is not null ||
        ...
        {
            throw new InvalidDataException(
                "History inline representation does not match its canonical byte count.");
        }

        return new ClipboardHistoryPayload(
            order,
            formatName,
            kind,
            count,
    ...
    private static Guid ReadNonEmptyGuid(
        string value,
        string field)
    {
