...
public sealed class ProtectedStorageCurrentSchemaV4MigrationTests
{
    ...
    public async Task Initialize_CreatesCurrentV4HistorySchemaAndLeavesCatalogV1()
    {
        ...
        try
        {
            ...
            ProtectedStorageDatabaseResult result = await service.InitializeOrValidateAsync(
                root, storageId, key, allowInitialize: true);
    ...
    public async Task Validate_MigratesCurrentV3ToV4AndPreservesIdentityAndPolicyRows()
    {
        ...
        try
        {
            CreateCurrentV3(factory, CurrentPath(root), key, storageId, applicationId, @"C:\Apps\Contoso.exe");
            CreateDatabaseIdentity(factory, CatalogPath(root), key, storageId, DatabaseRole.StorageCatalog, 1);
    ...
    public async Task Validate_InvalidCatalogDoesNotMutateCurrentV3()
    {
        ...
        try
        {
            CreateCurrentV3(factory, CurrentPath(root), key, storageId, DurableApplicationId.New(), @"C:\Apps\Classic.exe");
            CreateDatabaseIdentity(factory, CatalogPath(root), key, Guid.NewGuid(), DatabaseRole.StorageCatalog, 1);
    ...
    public async Task Validate_CurrentV2RunsThroughPolicyAndHistoryMigrations()
    {
        ...
        try
        {
            CreateCurrentV2(factory, CurrentPath(root), key, storageId, applicationId, @"C:\Apps\V2.exe");
            CreateDatabaseIdentity(factory, CatalogPath(root), key, storageId, DatabaseRole.StorageCatalog, 1);
    ...
    public async Task Validate_LegacyCurrentV1RunsThroughAllCurrentMigrations()
    {
        ...
        try
        {
            CreateDatabaseIdentity(factory, CurrentPath(root), key, storageId, DatabaseRole.Current, 1);
            CreateDatabaseIdentity(factory, CatalogPath(root), key, storageId, DatabaseRole.StorageCatalog, 1);
    ...
    private static void CreateCurrentV2(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, Guid storageId, DurableApplicationId applicationId, string executablePath)
    {
        CreateDatabaseIdentity(factory, path, key, storageId, DatabaseRole.Current, 2,
    ...
    private static void CreateCurrentV3(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, Guid storageId, DurableApplicationId applicationId, string executablePath)
    {
        CreateDatabaseIdentity(factory, path, key, storageId, DatabaseRole.Current, 3,
            (connection, transaction) =>
            {
                ApplicationIdentitySqlSchema.CreateTables(connection, transaction);
                ...
                appPolicy.CommandText = "INSERT INTO ApplicationCapturePolicy (ApplicationId, CaptureRule) VALUES ($applicationId, 'Allow');";
                ...
                formatPolicy.CommandText = "INSERT INTO ApplicationFormatCapturePolicy (ApplicationId, FormatName, CaptureRule, MaxBytes) VALUES ($applicationId, 'Text', 'Allow', 4096);";
                formatPolicy.Parameters.AddWithValue("$applicationId", applicationId.ToString());
                formatPolicy.ExecuteNonQuery();
            });
    }

    private static void CreateMalformedCurrentV3(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, Guid storageId)
    {
        CreateDatabaseIdentity(factory, path, key, storageId, DatabaseRole.Current, 3,
            (connection, transaction) =>
            {
                ...
                command.CommandText = """
                    ...
                    CREATE TABLE ApplicationFormatCapturePolicy (
                        ...
                        FOREIGN KEY (ApplicationId) REFERENCES ApplicationCapturePolicy(ApplicationId) ON DELETE CASCADE
                    );
                    """;
    ...
    private static void InsertApplicationWithPathAlias(SqliteConnection connection, SqliteTransaction transaction, DurableApplicationId applicationId, string executablePath)
    {
        ...
        using (SqliteCommand application = connection.CreateCommand())
        {
            ...
            application.CommandText = "INSERT INTO ApplicationIdentity (ApplicationId, CreatedAtUtc) VALUES ($applicationId, $createdAtUtc);";
        ...
        using (SqliteCommand alias = connection.CreateCommand())
        {
            ...
            alias.CommandText = "INSERT INTO ApplicationIdentityAlias (AliasType, AliasValue, ApplicationId, CreatedAtUtc) VALUES ($aliasType, $aliasValue, $applicationId, $createdAtUtc);";
    ...
    private static void CreateDatabaseIdentity(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, Guid storageId, DatabaseRole role, int schemaVersion, Action<SqliteConnection, SqliteTransaction>? configureTransaction = null)
    {
        ...
        using (SqliteCommand insert = connection.CreateCommand())
        {
            ...
            insert.CommandText = """
                INSERT INTO DatabaseIdentity (
                    ...
                    ArchiveBaseNumber, ArchiveSplitSequence, CoverageStartDate, CoverageEndDate)
                VALUES (1, $storageId, $databaseId, $role, $schemaVersion, 1, $createdAtUtc, NULL, NULL, NULL, NULL);
                """;
            ...
            insert.Parameters.AddWithValue("$createdAtUtc", DateTimeOffset.UtcNow.ToString("O", CultureInfo.InvariantCulture));
    ...
    private static int ReadSchemaVersion(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key)
    {
    ...
    private static int CountRows(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, string tableName)
    {
        using SqliteConnection connection = factory.Open(path, key, SqliteOpenMode.ReadOnly);
        using SqliteCommand command = connection.CreateCommand();
    ...
    private static bool HasTable(IKeyedSqliteConnectionFactory factory, string path, ReadOnlyMemory<byte> key, string tableName)
    {
        using SqliteConnection connection = factory.Open(path, key, SqliteOpenMode.ReadOnly);
        using SqliteCommand command = connection.CreateCommand();
    ...
    private static string CurrentPath(string root) => Path.Combine(root, "Current", "current.db");
    private static string CatalogPath(string root) => Path.Combine(root, "Current", "storage-catalog.db");
