...
$workRoot = Join-Path $repositoryRoot ".native-build\sqlcipher-arm64"
...
try {
    ...
    $vsInstall = (& $vsWhere -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.ARM64 -property installationPath).Trim()
    if ([string]::IsNullOrWhiteSpace($vsInstall)) {
        throw "Visual Studio ARM64 C++ toolchain not found."
    }
    ...
    $nmakeArgsPath = Join-Path $sqlcipherSource "clipensk-nmake-arm64.args"
    @"
FOR_WIN10=1
PLATFORM=arm64
USE_AMALGAMATION=1
...
"LTLIBS=Advapi32.lib User32.lib Kernel32.lib Crypt32.lib Ws2_32.lib Bcrypt.lib OneCore.lib"
...
cd /d "$opensslSource"
perl Configure VC-WIN64-ARM no-shared no-tests no-asm --prefix="$opensslInstall" --openssldir="$opensslInstall\ssl"
if errorlevel 1 exit /b %errorlevel%
nmake clean
if errorlevel 1 exit /b %errorlevel%
nmake
...
if errorlevel 1 exit /b %errorlevel%
nmake /f Makefile.msc sqlcipher.dll @clipensk-nmake-arm64.args
if errorlevel 1 exit /b %errorlevel%
...
"@
    ...
    $dumpbinExports = & cmd.exe /d /c "call `"$vcVars`" amd64_arm64 >nul && dumpbin /exports `"$sqlcipherDll`""
    if ($LASTEXITCODE -ne 0) {
    ...
    $dumpbinHeaders = & cmd.exe /d /c "call `"$vcVars`" amd64_arm64 >nul && dumpbin /headers `"$sqlcipherDll`""
    if ($LASTEXITCODE -ne 0) {
        throw "dumpbin /headers failed for sqlcipher.dll."
    }
    if (-not ($dumpbinHeaders -match "(?im)\baa64 machine \(ARM64\)")) {
        throw "sqlcipher.dll is not an ARM64 PE image."
    }
    ...
    Write-Host "Native SQLCipher ARM64 build complete."
